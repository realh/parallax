...
import org.parallax3d.renderer.TypeArray;
...
import org.parallax3d.core.Geometry.MorphNormal;
import org.parallax3d.core.Geometry.MorphTarget;
...
import org.parallax3d.math.Triangle;
...
public class Mesh extends GeometryObject
{
	...
	public void updateMorphTargets() {
		...
		if ( ((Geometry)this.getGeometry()).getMorphTargets() != null &&
				((Geometry)this.getGeometry()).getMorphTargets().size() > 0 ) {
	...
	public void raycast( Raycaster raycaster, List<Raycaster.Intersect> intersects) {
		...
		if ( geometry instanceof BufferGeometry ) 
		{
			...
			if ( bGeometry.getAttribute("index") != null ) {
				...
				for ( int oi = 0, ol = offsets.size(); oi < ol; ++oi ) 
				{
					...
					for ( int i = start, il = start + count; i < il; i += 3 ) 
					{
						...
						{
							intersectionPoint = _ray.intersectTriangle( _vA, _vB, _vC,
									material.getSides() != Material.SIDE.DOUBLE );
						}
						...
						if ( distance < precision || distance < raycaster.getNear() || distance > raycaster.getFar() )
							continue;
				...
				for ( int i = 0, j = 0, il = positions.getLength(); i < il; i += 3, j += 9 ) {
					...
					if ( material.getSides() == Material.SIDE.BACK ) {
						...
						intersectionPoint = _ray.intersectTriangle( _vA, _vB, _vC,
								material.getSides() != Material.SIDE.DOUBLE, null );
			...
			List<Material> objectMaterials = isFaceMaterial == true ?
					((MeshFaceMaterial)this.getMaterial()).getMaterials() : null;
			...
			for ( int f = 0, fl = aGeometry.getFaces().size(); f < fl; f ++ ) {
				...
				Material material = isFaceMaterial == true ?
						objectMaterials.get( face.getMaterialIndex() ) : this.getMaterial();
...
//	}
	...
	public void renderBuffer(WebGLRenderer renderer, WebGLGeometry geometryGroup, boolean updateBuffers)
	{
		...
		if ( getMaterial() instanceof HasWireframe && ((HasWireframe)getMaterial()).isWireframe() ) 
		{
			...
			if ( updateBuffers ) 
				gl.glBindBuffer(GL20.GL_ELEMENT_ARRAY_BUFFER, geometryGroup.__webglLineBuffer);
			
			gl.glDrawElements(GL20.GL_LINES, geometryGroup.__webglLineCount, GL20.GL_UNSIGNED_SHORT, 0);
		...
		{
			if ( updateBuffers ) 
				gl.glBindBuffer(GL20.GL_ELEMENT_ARRAY_BUFFER, geometryGroup.__webglFaceBuffer);
			
			gl.glDrawElements(GL20.GL_TRIANGLES, geometryGroup.__webglFaceCount, GL20.GL_UNSIGNED_SHORT, 0);
		}
...
//		Class UintArray = WebGLExtensions.get( gl, WebGLExtensions.Id.OES_element_index_uint )
// 						!= null && ntris > 21845
...
//			{
//				((BufferGeometry)geometryBuffer).setDirectBuffers( renderer.getGL(),
// 						BufferUsage.DYNAMIC_DRAW, !geometryBuffer.isDynamic() );
//			}
//
...
//	}
	...
	// setMeshBuffers
	public void setBuffers(GL20 gl, GeometryGroup geometryGroup,
						   int bufferUsageHint, boolean dispose, Material material)
	{
		//Log.debug("Called Mesh.setBuffers() - material=" + material.getId() + ", " +
		...
		{			 
			 ...
			 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglVertexBuffer);
			 ...
			 for ( int vk = 0, vkl = morphTargets.size(); vk < vkl; vk ++ ) 
			 {
				 ...
				 for ( int f = 0, fl = chunk_faces3.size(); f < fl; f ++ ) 
				 {
					 ...
					 if ( material instanceof HasSkinning && ((HasSkinning)material).isMorphNormals() ) 
					 {
						 ...
						 {
							 Geometry.VertextNormal faceVertexNormals =
									 morphNormals.get( vk ).vertexNormals.get( chf );
				 ...
				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglMorphTargetsBuffers.get(vk));
				 gl.glBufferData(GL20.GL_ARRAY_BUFFER, morphBuffer.getByteLength(),
                         morphBuffer.getBuffer(), bufferUsageHint);

				 if ( material instanceof HasSkinning && ((HasSkinning)material).isMorphNormals() ) 
				 {
					 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglMorphNormalsBuffers.get(vk));
		...
		{			 
			 ...
			 if ( offset_skin > 0 ) 
			 {
				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglSkinIndicesBuffer);
				 gl.glBufferData(GL20.GL_ARRAY_BUFFER, skinIndexArray.getByteLength(),
                         skinIndexArray.getBuffer(), bufferUsageHint);

				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglSkinWeightsBuffer);
				 gl.glBufferData(GL20.GL_ARRAY_BUFFER, skinWeightArray.getByteLength(),
		 ...
		 {		 
			 for ( int f = 0, fl = chunk_faces3.size(); f < fl; f ++ ) 
			 {
				 ...
				 if ( vertexColors.size() == 3 &&
						 ((HasVertexColors)material).isVertexColors() == Material.COLORS.VERTEX)
				 {
			 ...
			 if ( offset_color > 0 ) 
			 {
				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglColorBuffer);
		 ...
		 {			 
			 ...
			 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglTangentBuffer);
			 gl.glBufferData(GL20.GL_ARRAY_BUFFER, tangentArray.getByteLength(),
		 ...
		 if ( dirtyNormals ) 
		 {
			 ...
			 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglNormalBuffer);
			 gl.glBufferData(GL20.GL_ARRAY_BUFFER, normalArray.getByteLength(),
		 ...
		 if ( dirtyUvs && obj_uvs != null &&  obj_uvs.size() > 0 ) 
		 {
			 ...
			 if ( offset_uv > 0 ) 
			 {
				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglUVBuffer);
				 gl.glBufferData(GL20.GL_ARRAY_BUFFER, uvArray.getByteLength(),
		 ...
		 if (dirtyUvs && obj_uvs2 != null && obj_uvs2.size() > 0 ) 
		 {
			 ...
			 if ( offset_uv2 > 0 ) 
			 {
				 gl.glBindBuffer(GL20.GL_ARRAY_BUFFER, geometryGroup.__webglUV2Buffer);
				 gl.glBufferData(GL20.GL_ARRAY_BUFFER, uv2Array.getByteLength(),
		 ...
		 {			 
			 ...
			 gl.glBindBuffer(GL20.GL_ELEMENT_ARRAY_BUFFER, geometryGroup.__webglFaceBuffer);
			 gl.glBufferData(GL20.GL_ELEMENT_ARRAY_BUFFER, faceArray.getByteLength(),
                     faceArray.getBuffer(), bufferUsageHint);

			 gl.glBindBuffer(GL20.GL_ELEMENT_ARRAY_BUFFER, geometryGroup.__webglLineBuffer);
			 gl.glBufferData(GL20.GL_ELEMENT_ARRAY_BUFFER, lineArray.getByteLength(),
				 ...
				 if ( customAttribute.size == 1 ) 
				 {
							 ...
							 || customAttribute.getBoundTo() == Attribute.BOUND_TO.VERTICES ) 
					 {
						 for ( int f = 0, fl = chunk_faces3.size(); f < fl; f ++ ) 
						 {
							 ...
							 customAttribute.array.set(offset_custom,
									 (Double)customAttribute.getValue().get(face.getA()));
							 customAttribute.array.set(offset_custom + 1,
									 (Double)customAttribute.getValue().get(face.getB()));
							 customAttribute.array.set(offset_custom + 2,
									 (Double)customAttribute.getValue().get(face.getC()));
				 ...
				 else if ( customAttribute.size == 3 ) 
				 {
					 ...
					 else if ( customAttribute.getBoundTo() == Attribute.BOUND_TO.FACE_VERTICES) 
					 {
						 for ( int f = 0, fl = chunk_faces3.size(); f < fl; f ++ ) 
						 {
							 if(customAttribute.type == Attribute.TYPE.C) 
							 {
								 List<Color> value = (List<Color>)
										 customAttribute.getValue().get(chunk_faces3.get(f));
							 ...
							 {
								 List<Vector3> value = (List<Vector3>)
										 customAttribute.getValue().get(chunk_faces3.get(f));
...
//				geometry.getGeometryGroupsCache().put(groupHash,
// 						new GeometryGroup(materialIndex, numMorphTargets, numMorphNormals));
//
